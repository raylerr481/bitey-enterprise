...
export async function onRequest(context) {
  ...
  try {
    ...
    const state = passed ? "READY" : "TESTING";

    const results = await Promise.all(Object.entries(checks).map(([checkKey, value]) => storage.setReadinessCheck(company.id, assistant.id, checkKey, {
      passed: value,
      details: { source: "server", evaluated_state: state },
      checked_at: new Date().toISOString(),
    })));

    return json({ ok: true, data: { state, activation_allowed: false, production_activation: false, checks, results }, request_id: requestId });
  } catch (error) {
    ...
    return json({ ok: false, error: { code: error.code || "INTERNAL_ERROR", message: status === 503 ? "Enterprise persistence is not configured correctly yet." : "Enterprise readiness check failed." }, request_id: requestId }, status);
  }
}
